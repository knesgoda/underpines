...
const GroveMemberDetail = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: p } = await supabase.from('profiles').select('id, handle, display_name, bio, mantra, city, country_code, created_at, updated_at, avatar_url, default_avatar_key, header_image_url, account_status, is_age_verified, seedling_ends_at, last_seen_at').eq('handle', handle).single();
